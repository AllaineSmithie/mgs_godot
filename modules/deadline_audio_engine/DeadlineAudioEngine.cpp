...
void DeadlineAudioEngine::_bind_methods()
{
	...
	ClassDB::bind_method(D_METHOD("remove_instrument", "instrument"), &DeadlineAudioEngine::remove_instrument);
}
...
void DeadlineAudioEngine::on_bus_layout_changed()
{
	...
	for (auto i = 0; i < AudioServer::get_singleton()->get_bus_count(); ++i)
	{
		...
		if (!found)
		{
			print_line("Bus " + AudioServer::get_singleton()->get_bus_name(i) + " Synth added. UUID: " + AudioServer::get_singleton()->get_bus_uuid(i));
...
bool DeadlineAudioEngine::remove_instrument()
{
	return true;
}
