#ifndef JUCE_PLATFORM_DEFS_H
#define JUCE_PLATFORM_DEFS_H
#include <assert.h>
 ...
 */
#define ERR_FAIL_COND_FALSE(m_cond)																		\
	if (unlikely(m_cond))																				\
		((void)0);																						\
	else {                                                                                              \
		_err_print_error(FUNCTION_STR, __FILE__, __LINE__, "Condition \"" _STR(m_cond) "\" is false.");  \
		return;                                                                                         \
	}
...
#define DBG(textToWrite)              //JUCE_BLOCK_WITH_FORCED_SEMICOLON (juce::String tempDbgBuf; tempDbgBuf << textToWrite; juce::Logger::outputDebugString (tempDbgBuf);)
  ...
  */
#if JUCE_DEBUG
#define jassertfalse                  assert(false);//JUCE_BLOCK_WITH_FORCED_SEMICOLON (JUCE_LOG_CURRENT_ASSERTION; if (juce::juce_isRunningUnderDebugger()) JUCE_BREAK_IN_DEBUGGER; JUCE_ANALYZER_NORETURN)
#else
#define jassertfalse                  assert(false);//JUCE_BLOCK_WITH_FORCED_SEMICOLON (JUCE_LOG_CURRENT_ASSERTION; if (juce::juce_isRunningUnderDebugger()) JUCE_BREAK_IN_DEBUGGER; JUCE_ANALYZER_NORETURN)
#endif
  ...
  /** Platform-independent assertion macro.
	  ...
	  @see jassertfalse
  */
#define jassert(expression)           ERR_FAIL_COND_FALSE(expression) // JUCE_BLOCK_WITH_FORCED_SEMICOLON (if (! (expression)) jassertfalse;)
  ...
  */
#define jassertquiet(expression)      ERR_FAIL_COND_FALSE(expression) // JUCE_BLOCK_WITH_FORCED_SEMICOLON (if (! (expression)) jassertfalse;)
...
#define jassertquiet(expression)     ERR_FAIL_COND_FALSE(expression) // JUCE_BLOCK_WITH_FORCED_SEMICOLON (if (! (expression)) jassertfalse;)
#else
#define jassert(expression)          ERR_FAIL_COND_FALSE(expression)//JUCE_BLOCK_WITH_FORCED_SEMICOLON ( ; )
#define jassertquiet(expression)     ERR_FAIL_COND_FALSE(expression) //JUCE_BLOCK_WITH_FORCED_SEMICOLON (if (false) (void) (expression);)
#endif
