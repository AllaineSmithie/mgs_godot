	...
	uint32* BigInteger::ensureSize(const size_t numVals)
	{
		if (numVals > allocatedSize)
		{
			...
			if (heapAllocation == nullptr)
			{
				...
				memcpy(heapAllocation.operator void *(), preallocated, sizeof(uint32) * numPreallocatedInts);
			}
			else
			{
...
#pragma intrinsic (_BitScanReverse)
		...
		BigInteger& x, BigInteger& y)
	{
		...
		{
			auto& v = tempValues.get(tempValues.size() - i - 1);
	...
	String BigInteger::toString(const int base, const int minimumNumCharacters) const
	{
		...
		if (base == 2 || base == 8 || base == 16)
		{
			...
			for (;;)
			{
				...
				s = String((const wchar_t*)(uint8)hexDigits[remainder]) + s;
			}
		}
		else if (base == 10)
		{
			...
			for (;;)
			{
				...
				s = String::num(remainder.getBitRangeAsInt(0, 8), 0) + s;
			}
		}
		else
		{
		...
		s = s.pad_zeros(minimumNumCharacters);
