...
#define JUCE_LOAD_SRC_DEST(srcLoad, dstLoad)                    const Mode::ParallelType d = dstLoad (dest), s = srcLoad (src);
		...
		union signMask32 { float  f; juce::uint32 i; };
		union signMask64 { double d; juce::uint64 i; };
		...
		namespace
		{
			...
			template <typename Size>
			juce::Range<float> findMinAndMax(const float* src, Size num) noexcept
			{
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
			...
			template <typename Size>
			juce::Range<double> findMinAndMax(const double* src, Size num) noexcept
			{
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
	...
	void JUCE_CALLTYPE juce::FloatVectorOperations::convertFixedToFloat(float* dest, const int* src, float multiplier, size_t num) noexcept
	{
