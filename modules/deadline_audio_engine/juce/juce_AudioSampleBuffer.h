#ifndef JUCE_AUDIO_SAMPLE_BUFFER_H
#define JUCE_AUDIO_SAMPLE_BUFFER_H
	...
	*/
	...
	public:
			...
			if (newNumSamples != size || newNumChannels != numChannels)
			{
				...
				if (keepExistingContent)
				{
					...
					else
					{
						...
						auto numSamplesToCopy = (size_t)MIN(newNumSamples, size);
						...
						if (!isClear)
						{
							auto numChansToCopy = MIN(numChannels, newNumChannels);
		...
		*/
		...
		{
#ifdef JUCE_DEBUG
			ERR_FAIL_COND(startSample < 0);
			ERR_FAIL_COND(numSamples < 0);
			ERR_FAIL_INDEX(startSample + numSamples, size + 1);
#endif // JUCE_DEBUG
		...
		*/
		void clear(int channel, int startSample, int numSamples) noexcept
		{
			...
			ERR_FAIL_COND(startSample < 0);
			ERR_FAIL_COND(numSamples < 0);
			ERR_FAIL_INDEX(channel, numChannels);
			ERR_FAIL_INDEX(startSample + numSamples, size + 1);
#endif // JUCE_DEBUG
		...
		*/
		...
		{
#ifdef JUCE_DEBUG
			ERR_FAIL_INDEX_V(channel, numChannels, (Type)0);
			ERR_FAIL_INDEX_V(sampleIndex, size, (Type)0);
#endif // JUCE_DEBUG
		...
		*/
		...
		{
#ifdef JUCE_DEBUG
			ERR_FAIL_INDEX(destChannel, numChannels);
			ERR_FAIL_INDEX(destSample, size);
#endif // JUCE_DEBUG
		...
		*/
		juce::Range<Type> findMinMax(int channel, int startSample, int numSamples) const noexcept
		{
			jassert(isPositiveAndBelow(channel, numChannels));
			jassert(startSample >= 0 && numSamples >= 0 && startSample + numSamples <= size);
...
#endif // JUCE_AUDIO_SAMPLE_BUFFER_H
