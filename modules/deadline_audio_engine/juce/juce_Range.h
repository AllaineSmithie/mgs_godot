...
namespace juce
{
	...
	*/
	...
	public:
		...
		constexpr Range(const Variant startValue, const Variant endValue) noexcept
		...
		constexpr static Range between(const Variant position1, const Variant position2) noexcept
		{
		...
		[[nodiscard]] static Range withStartAndLength(const Variant startValue, const Variant length) noexcept
		{
			jassert(length >= Variant());
			return Range(startValue, startValue + length);
		}

		/** Returns a range with the specified start position and a length of zero. */
		[[nodiscard]] constexpr static Range emptyRange(const Variant start) noexcept
		{
		...
		constexpr inline Variant getStart() const noexcept { return start; }

		/** Returns the length of the range. */
		constexpr inline Variant getLength() const noexcept { return end - start; }

		/** Returns the end of the range. */
		constexpr inline Variant getEnd() const noexcept { return end; }
		...
		*/
		void setStart(const Variant newStart) noexcept
		{
			start = newStart;
			if (end < newStart)
				end = newStart;
		...
		*/
		[[nodiscard]] constexpr Range withStart(const Variant newStart) const noexcept
		{
			return Range(newStart, jmax(newStart, end));
		}

		/** Returns a range with the same length as this one, but moved to have the given start position. */
		[[nodiscard]] constexpr Range movedToStartAt(const Variant newStart) const noexcept
		{
		...
		*/
		void setEnd(const Variant newEnd) noexcept
		{
			end = newEnd;
			if (newEnd < start)
				start = newEnd;
		...
		/** Returns a range with the same start position as this one, but a different end.
			...
			will be pushed back to equal the new end point.
		*/
		[[nodiscard]] constexpr Range withEnd(const Variant newEnd) const noexcept
		{
		...
		[[nodiscard]] constexpr Range movedToEndAt(const Variant newEnd) const noexcept
		{
		...
		*/
		void setLength(const Variant newLength) noexcept
		{
			end = start + jmax(Variant(), newLength);
		}

		/** Returns a range with the same start as this one, but a different length.
			Lengths less than zero are treated as zero.
		*/
		[[nodiscard]] constexpr Range withLength(const Variant newLength) const noexcept
		{
		...
		*/
		[[nodiscard]] constexpr Range expanded(Variant amount) const noexcept
		{
		...
		inline Range operator+= (const Variant amountToAdd) noexcept
		{
			start += amountToAdd;
			end += amountToAdd;
			return *this;
		...
		inline Range operator-= (const Variant amountToSubtract) noexcept
		{
		...
		/** Returns a range that is equal to this one with an amount added to its
			start and end.
		*/
		constexpr Range operator+ (const Variant amountToAdd) const noexcept
		{
		...
		constexpr Range operator- (const Variant amountToSubtract) const noexcept
		{
		...
		*/
		constexpr bool contains(const Variant position) const noexcept
		{
			return start <= position && position < end;
		}

		/** Returns the nearest value to the one supplied, which lies within the range. */
		Variant clipValue(const Variant value) const noexcept
		{
		...
		[[nodiscard]] constexpr Range getUnionWith(const Variant valueToInclude) const noexcept
		{
		...
		*/
		...
		{
			const Variant otherLen = rangeToConstrain.getLength();
		...
		static Range findMinAndMax(const Variant* values, Integral numValues) noexcept
		{
			if (numValues <= 0)
				return Range();

			const Variant first(*values++);
			Range r(first, first);
