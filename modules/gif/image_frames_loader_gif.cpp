...
Error ImageFramesLoaderGIF::load_image_frames(Ref<ImageFrames> &r_image_frames, const Ref<FileAccess> &f, int max_frames) const {
	GifLoader gif;
	...
	do { // Parse every record of the GIF.
		...
		switch (recordType) {
			...
			case IMAGE_DESC_RECORD_TYPE: { // Record with the image data.
				...
				//PackedByteArray::Write data_write = frame_data.write();
				...
				switch (gcb.DisposalMode) { // What should happen after the frame has been drawn.
					...
					case DISPOSE_PREVIOUS: { // Reset the screen to the last undisposed frame.
						...
						if (last_undisposed_frame == -1) { // Clear the frame.
							...
							//PackedByteArray::Read last_frame_read = last_frame_data.read();
							for (int y = 0; y < imageDesc.Height; y++) {
								int write_y = y + imageDesc.Top;
								int write_index = (write_y * gif->SWidth + imageDesc.Left) * RGBA;
...
Error GifLoader::load_from_file_access(Ref<ImageFrames> &r_image_frames, const Ref<FileAccess> &f, int max_frames) {
	Error err;
	ERR_FAIL_COND_V(f.is_null(), Error::ERR_FILE_BAD_PATH);
